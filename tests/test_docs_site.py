from __future__ import annotations

import tempfile
import unittest
from pathlib import Path

from sqlbench_lab.docs_site import build_docs_site


class DocsSiteTests(unittest.TestCase):
    def test_build_docs_site_writes_dense_browser_pages(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            output_dir = Path(tmp_dir) / "site"

            summary = build_docs_site(output_dir)

            index_html = (output_dir / "index.html").read_text(encoding="utf-8")
            training_html = (output_dir / "training.html").read_text(encoding="utf-8")
            learnings_html = (output_dir / "learnings.html").read_text(encoding="utf-8")
            research_html = (output_dir / "research.html").read_text(encoding="utf-8")
            pipeline_html = (output_dir / "pipeline.html").read_text(encoding="utf-8")
            runbook_html = (output_dir / "runbook.html").read_text(encoding="utf-8")
            serving_html = (output_dir / "serving.html").read_text(encoding="utf-8")
            evaluation_html = (output_dir / "evaluation.html").read_text(encoding="utf-8")
            livesqlbench_html = (output_dir / "livesqlbench.html").read_text(encoding="utf-8")
            observability_html = (output_dir / "observability.html").read_text(encoding="utf-8")
            documentation_html = (output_dir / "documentation.html").read_text(encoding="utf-8")
            experiment_html = (
                output_dir
                / "experiments"
                / "qwen35_0_8b__exp028_trl_regional_sales_unit_price_slot_v3.html"
            ).read_text(encoding="utf-8")

        self.assertGreaterEqual(summary.page_count, 9)
        self.assertEqual(summary.asset_count, 1)
        self.assertIn("SQLBench Lab", index_html)
        self.assertIn("System Map", index_html)
        self.assertIn("Iterative Supervised FT", index_html)
        self.assertIn("student.initial_adapter_dir", index_html)
        self.assertIn("Exp031", index_html)
        self.assertIn("restaurant plus airline as prompt-dev", index_html)
        self.assertIn("works_cycles plus public_review_platform", index_html)
        self.assertIn("7/50", index_html)
        self.assertIn("Recent Experiment Ledger", training_html)
        self.assertIn("iterative supervised fine-tuning", training_html)
        self.assertIn("initial_adapter_dir", training_html)
        self.assertIn("Iterate With Lineage", training_html)
        self.assertIn("Exp028", training_html)
        self.assertIn("Practical fine-tuning lessons", learnings_html)
        self.assertIn("Canonical slot placement worked", learnings_html)
        self.assertIn("Exp029", learnings_html)
        self.assertIn("Exp030", learnings_html)
        self.assertIn("Exp031", learnings_html)
        self.assertIn("Exp032 plan", learnings_html)
        self.assertIn("Exp032 manual candidates", learnings_html)
        self.assertIn("Exp033 setup", learnings_html)
        self.assertIn("Exp036 setup", learnings_html)
        self.assertIn("Exp037 and Exp038 setup", learnings_html)
        self.assertIn("Qwen2.5-Coder-1.5B", learnings_html)
        self.assertIn("Exp049 QLoRA", learnings_html)
        self.assertIn("Exp048 vs Exp049 stress", learnings_html)
        self.assertIn("Exp050-057 one-DB ladder", learnings_html)
        self.assertIn("support no-issue filters", learnings_html)
        self.assertIn("Exp056 combined the supplements into train_v4", learnings_html)
        self.assertIn("22/24 challenge_v1", learnings_html)
        self.assertIn("Exp057 keeps QLoRA as a credible memory/runtime tradeoff", learnings_html)
        self.assertIn("Train_v4 has no exact question or gold-SQL overlap", learnings_html)
        self.assertIn("Exp058-063 contrast ladder", learnings_html)
        self.assertIn("challenge_v2", learnings_html)
        self.assertIn("anti_join/left_join_predicate at 1/5", learnings_html)
        self.assertIn("train_v5 with 236 rows", learnings_html)
        self.assertIn("12/15 challenge_v2", learnings_html)
        self.assertIn("Exp062 is a rejected ablation", learnings_html)
        self.assertIn("Exp065 iterative SFT", learnings_html)
        self.assertIn("Reject Exp065 and keep Exp056 promoted", learnings_html)
        self.assertIn("Blacksmith reset", learnings_html)
        self.assertIn("c000_current_system", learnings_html)
        self.assertIn("c001_schema_grounded", learnings_html)
        self.assertIn("every candidate is tracked in MLflow", learnings_html)
        self.assertIn("Expansion alone is not enough", learnings_html)
        self.assertIn("Text-to-SQL Research Map", research_html)
        self.assertIn("Blacksmith Moves", research_html)
        self.assertIn("safe environment step", research_html)
        self.assertIn("OpenEnv", research_html)
        self.assertIn("Automatic Metadata Extraction", research_html)
        self.assertIn("Candidate Pool + Execution + Selection", research_html)
        self.assertIn("Verified Iterative Supervised Fine-Tuning Data Factory", research_html)
        self.assertIn("The Death of Schema Linking?", research_html)
        self.assertIn("SPFT-SQL", research_html)
        self.assertIn("STaR-SQL", research_html)
        self.assertIn("CoTE-SQL", research_html)
        self.assertIn("Schema on the Inside", research_html)
        self.assertIn("FINER-SQL", research_html)
        self.assertIn("Progress-SQL", research_html)
        self.assertIn("Reinforcing Code Generation", research_html)
        self.assertIn("verified open-dev failures", research_html)
        self.assertIn("Modern Fine-Tuning Pipeline", research_html)
        self.assertIn("iterative supervised fine-tuning", research_html)
        self.assertIn("not online RL", research_html)
        self.assertIn("competition readiness", research_html)
        self.assertIn("regressed stable eval_v1 from 12/12 to 10/12", research_html)
        self.assertIn("Training Hygiene Rules", research_html)
        self.assertIn("MLOps Run Contract", pipeline_html)
        self.assertIn("sqlbench_lab.mlops.run_contract", pipeline_html)
        self.assertIn("dev environment boundary for TAP-648", pipeline_html)
        self.assertIn("gs://mistri-sqlbench-dev-artifacts", pipeline_html)
        self.assertIn("Any non-dev environment fails fast", pipeline_html)
        self.assertIn("Exp056 as promotable", pipeline_html)
        self.assertIn("Local Dev Metaflow Flow", pipeline_html)
        self.assertIn("flows/sql_adapter_offline_dev_flow.py", pipeline_html)
        self.assertIn("uv run --group mlops python flows/sql_adapter_offline_dev_flow.py run", pipeline_html)
        self.assertIn("start_temp_dev_endpoint", pipeline_html)
        self.assertIn("--endpoint-eval-result", pipeline_html)
        self.assertIn("--run-load-test", pipeline_html)
        self.assertIn("Replay mode is intentional", pipeline_html)
        self.assertIn("Dev GCS Sync Plan", pipeline_html)
        self.assertIn("sql_adapter_gcs_sync_plan:v1", pipeline_html)
        self.assertIn("gcs_sync_plan", pipeline_html)
        self.assertIn("Dev CLI Container", pipeline_html)
        self.assertIn("docker/sqlbench-dev-cli.Dockerfile", pipeline_html)
        self.assertIn("cloudbuild/sqlbench-dev-cli.yaml", pipeline_html)
        self.assertIn("sqlbench-lab-dev-cli:dev", pipeline_html)
        self.assertIn("docker/sqlbench-vllm.Dockerfile", pipeline_html)
        self.assertIn("cloudbuild/sqlbench-vllm.yaml", pipeline_html)
        self.assertIn("SQLBENCH_ADAPTER_URI", pipeline_html)
        self.assertIn("SQLBENCH_BASE_SERVED_MODEL", pipeline_html)
        self.assertIn("LoRA request model", pipeline_html)
        self.assertIn("Requesting the base alias evaluates the base model", pipeline_html)
        self.assertIn("SQLBENCH_BASE_MODEL_URI", pipeline_html)
        self.assertIn("sql_adapter_dev_base_model_mirror:v1", pipeline_html)
        self.assertIn("torch 2.11.0+cu130", pipeline_html)
        self.assertIn("Cloud Run L4 attempt failed", pipeline_html)
        self.assertIn("driver <code>12020</code>", pipeline_html)
        self.assertIn("rejects <code>cloud_run_gpu</code>", pipeline_html)
        self.assertIn("defaults to <code>gce_gpu_vm</code>", pipeline_html)
        self.assertIn("--serving-target gce_gpu_vm", pipeline_html)
        self.assertIn("common-cu129-ubuntu-2404-nvidia-580", pipeline_html)
        self.assertIn("compute.googleapis.com/gpus_all_regions", pipeline_html)
        self.assertIn("regional <code>NVIDIA_L4_GPUS</code> quota alone is not enough", pipeline_html)
        self.assertIn("<code>local_gpu_docker</code>", pipeline_html)
        self.assertIn("docker run --gpus all", pipeline_html)
        self.assertIn("SQLBENCH_VLLM_EXTRA_ARGS", pipeline_html)
        self.assertIn("<code>file://</code> URIs", pipeline_html)
        self.assertIn("not a prod serving image", pipeline_html)
        self.assertIn("Dev Cloud Training, Serving, Registry, and Hardening Contracts", pipeline_html)
        self.assertIn("mlops dev-cloud-plan", pipeline_html)
        self.assertIn("sql_adapter_vertex_training_job:v1", pipeline_html)
        self.assertIn("vertex_training_job_plan", pipeline_html)
        self.assertIn("sql_adapter_dev_vllm_endpoint:v1", pipeline_html)
        self.assertIn("dev_endpoint_plan", pipeline_html)
        self.assertIn("sql_adapter_dev_promotion_registry:v1", pipeline_html)
        self.assertIn("promotion_registry_plan", pipeline_html)
        self.assertIn("artifact manifest with file/tree SHA-256 hashes", pipeline_html)
        self.assertIn("sql_adapter_dev_observability:v1", pipeline_html)
        self.assertIn("endpoint_monitoring_record", pipeline_html)
        self.assertIn("logs URI, startup time, GPU/KV-cache notes, failure mode", pipeline_html)
        self.assertIn("generated SQL character-count", pipeline_html)
        self.assertIn("Token counts are not claimed", pipeline_html)
        self.assertIn("sql_adapter_dev_cost_capacity:v1", pipeline_html)
        self.assertIn("sorted concurrency ladder", pipeline_html)
        self.assertIn("Commands are documented", runbook_html)
        self.assertIn("Optimize prompt candidate", runbook_html)
        self.assertIn("Report token lengths", runbook_html)
        self.assertIn("Filter train token budget", runbook_html)
        self.assertIn("Evaluate candidate pool", runbook_html)
        self.assertIn("Continue best adapter", runbook_html)
        self.assertIn("initial_adapter_loaded=true", runbook_html)
        self.assertIn("Step SQL environment", runbook_html)
        self.assertIn("db_sql_agent_env.cli env-step", runbook_html)
        self.assertIn("Run extracted SQL agent env", runbook_html)
        self.assertIn("separate_projects/db_sql_agent_env", runbook_html)
        self.assertIn("Import standalone seed data", runbook_html)
        self.assertIn("db_sql_agent_env.cli import-seed", runbook_html)
        self.assertIn("Evaluate execution-guided repair", runbook_html)
        self.assertIn("Every MIPROv2/GEPA candidate has MLflow tags", runbook_html)
        self.assertIn("Execution-Guided Repair Experiment", evaluation_html)
        self.assertIn("exp064_candidates_challenge_v2", evaluation_html)
        self.assertIn("Compete with iterative supervised fine-tuning", livesqlbench_html)
        self.assertIn("Iterative Supervised Fine-Tuning Loop", livesqlbench_html)
        self.assertIn("LiveSQLBench-CLI", livesqlbench_html)
        self.assertIn("LiveSQLBench-Agent", livesqlbench_html)
        self.assertIn("Base-Lite-SQLite", livesqlbench_html)
        self.assertIn("Submission Boundary", livesqlbench_html)
        self.assertIn("No GT/test-case package is needed", livesqlbench_html)
        self.assertIn("Website score", livesqlbench_html)
        self.assertIn("Verified badge", livesqlbench_html)
        self.assertIn("Local supervised iteration", livesqlbench_html)
        self.assertIn("Implemented Submission Path", livesqlbench_html)
        self.assertIn("sql livesqlbench-prepare", livesqlbench_html)
        self.assertIn("public/livesqlbench-base-lite/livesqlbench_data.jsonl", livesqlbench_html)
        self.assertIn("official-runner artifacts", livesqlbench_html)
        self.assertIn("Continuation manifests preserve adapter lineage", livesqlbench_html)
        self.assertIn("Deployment quality, concurrency, and cost are separate gates.", serving_html)
        self.assertIn("Local Adapter Inference", serving_html)
        self.assertIn("QLoRA Serving Check", serving_html)
        self.assertIn("Exp049 QLoRA also served successfully but stayed at 9/12", serving_html)
        self.assertIn("LoRA vs QLoRA Stress", serving_html)
        self.assertIn("1.4715", serving_html)
        self.assertIn("no QLoRA-specific inference stress penalty", serving_html)
        self.assertIn("Raw Serving Notebook", serving_html)
        self.assertIn("notebooks/sql_local_serving_kv_cache_walkthrough.ipynb", serving_html)
        self.assertIn("Local SQL Ask App", serving_html)
        self.assertIn("web query-app", serving_html)
        self.assertIn("read-only SELECT execution", serving_html)
        self.assertIn("Repo Endpoint Evaluation", serving_html)
        self.assertIn("GPT-5.1 API Inference", serving_html)
        self.assertIn('"reasoning": {"effort": "medium"}', serving_html)
        self.assertIn("GPT-5.1 Medium Reasoning Cost", serving_html)
        self.assertIn("100k / Week", serving_html)
        self.assertIn("c160", serving_html)
        self.assertIn("$3.4k/month", serving_html)
        self.assertIn("optimizer</span>", observability_html)
        self.assertIn("prompt_dev_dataset", observability_html)
        self.assertIn("Markdown docs are allowed", documentation_html)
        self.assertIn("Use browser docs for durable operating surfaces", documentation_html)
        self.assertIn("Train Inputs", experiment_html)
        self.assertIn("Eval Results", experiment_html)
