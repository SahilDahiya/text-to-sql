    ...
    body = f"""
        ...
        <section class="panel full">
          ...
          <table class="key-table">
            ...
            <tr><th>Cloud target</th><td>Do not promote Cloud Run GPU for this image. The dev endpoint planner now rejects <code>cloud_run_gpu</code> and defaults to <code>gce_gpu_vm</code>, with <code>gke_gpu_node_pool</code> and <code>vertex_custom_gpu_endpoint</code> as other driver-controlled targets. Use a target where the NVIDIA driver can satisfy CUDA 13.0, or build a separate CUDA 12.x serving stack and rerun endpoint eval plus load gates before promotion.</td></tr>
            <tr><th>GCE preflight</th><td>The first <code>gce_gpu_vm</code> create attempt used <code>g2-standard-4</code>, one <code>nvidia-l4</code>, and image family <code>common-cu129-ubuntu-2404-nvidia-580</code>. It failed before allocation because project quota <code>compute.googleapis.com/gpus_all_regions</code> is <code>0</code>. Raise global GPU quota to at least <code>1</code> before retrying; regional <code>NVIDIA_L4_GPUS</code> quota alone is not enough.</td></tr>
            <tr><th>Local rehearsal</th><td><code>local_gpu_docker</code> is the practical prod-like rehearsal path while GCE GPU quota is blocked. It runs the same <code>sqlbench-vllm:dev</code> image with <code>docker run --gpus all</code>, maps <code>8000:8000</code>, mounts local GCP ADC at <code>/root/.config/gcloud</code>, sets <code>GOOGLE_CLOUD_PROJECT</code>, and passes the same <code>SQLBENCH_BASE_MODEL_URI</code>, <code>SQLBENCH_ADAPTER_URI</code>, and <code>SQLBENCH_VLLM_EXTRA_ARGS</code> env contract before endpoint eval and load gates. If ADC cannot read the dev model bucket, bind-mount the already downloaded base model and adapter and use <code>file://</code> URIs for a local-only rehearsal.</td></tr>
            <tr><th>Required env</th><td><code>SQLBENCH_BASE_MODEL</code>, <code>SQLBENCH_BASE_SERVED_MODEL</code>, <code>SQLBENCH_OPENAI_MODEL</code>, <code>SQLBENCH_ADAPTER_NAME</code>, and <code>SQLBENCH_ADAPTER_URI</code>.</td></tr>
            <tr><th>LoRA request model</th><td><code>SQLBENCH_OPENAI_MODEL</code> is the client-facing model ID used by endpoint eval and load tests, and it must equal <code>SQLBENCH_ADAPTER_NAME</code>. <code>SQLBENCH_BASE_SERVED_MODEL</code> is only the base model alias passed to vLLM <code>--served-model-name</code>. Requesting the base alias evaluates the base model, not the LoRA adapter.</td></tr>
        ...
        <section class="panel full">
          ...
          <table class="key-table">
            <tr><th>CLI</th><td><code>uv run python -m sqlbench_lab.cli mlops dev-cloud-plan --run-id &lt;run_id&gt; --vertex-dry-run --vertex-machine-type n1-standard-4 --vertex-accelerator-count 0 --serving-target gce_gpu_vm --publish-local-dir artifacts/dev/&lt;run_id&gt;/publish --publish-gcs --update-current-pointer --output artifacts/dev/cloud_bundle.json --vertex-config-output artifacts/dev/vertex_custom_job.json</code></td></tr>
            <tr><th>TAP-636</th><td><code>sql_adapter_vertex_training_job:v1</code> builds a Vertex AI Custom Job plan around the dev CLI image, the durable GCS manifest URI, the container manifest path used by <code>sql run-sft</code>, the dev training service account, and an explicit GPU machine spec. The companion gcloud command uses <code>gcloud ai custom-jobs create --config</code>.</td></tr>
            <tr><th>TAP-637</th><td><code>sql_adapter_dev_vllm_endpoint:v1</code> records the temporary GCP vLLM endpoint plan: base model, LoRA adapter URI, base served model alias, client-facing LoRA OpenAI model name, GPU type/count, max model length, max sequences, and health/completions paths.</td></tr>
            <tr><th>TAP-638</th><td><code>sql_adapter_dev_promotion_registry:v1</code> records the dev adapter version, metadata URI, current pointer, rollback pointer, run contract URI, and promotion decision. <code>sql_adapter_dev_cloud_publish:v1</code> writes the generated metadata, emits an artifact manifest with file/tree SHA-256 hashes, and refuses to update <code>current.json</code> for rejected runs before upload work starts.</td></tr>
            ...
            <tr><th>TAP-643</th><td><code>sql_adapter_dev_endpoint_monitoring:v1</code> records endpoint quality and runtime signals: serving target, base model, endpoint image, logs URI, startup time, GPU/KV-cache notes, failure mode, request count, concurrency, success/error/timeout counts, p50/p95/p99 latency, generated SQL character-count min/p50/p95/max/mean, endpoint pass rate, empty SQL count, and SQL failure buckets. Token counts are not claimed unless the serving artifact records them.</td></tr>
            <tr><th>TAP-645</th><td><code>sql_adapter_dev_cost_capacity:v1</code> records training GPU capacity, endpoint GPU capacity, uptime, caller-supplied hourly cost rates, estimated cost, request count, peak concurrency, RPS, and a sorted concurrency ladder when multiple load-test artifacts are attached.</td></tr>
