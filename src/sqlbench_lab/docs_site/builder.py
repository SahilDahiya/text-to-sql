...
def _render_pipeline() -> str:
    ...
    body = f"""
        ...
        <section class="panel full">
          ...
          <table class="key-table">
            <tr><th>CLI</th><td><code>uv run python -m sqlbench_lab.cli mlops dev-cloud-plan --run-id &lt;run_id&gt; --vertex-dry-run --publish-local-dir artifacts/dev/&lt;run_id&gt;/publish --publish-gcs --update-current-pointer --output artifacts/dev/cloud_bundle.json --vertex-config-output artifacts/dev/vertex_custom_job.json</code></td></tr>
